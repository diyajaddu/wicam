...
NmcliWireless::NmcliWireless(const QString &inteface_dev)
{
    this->interface(inteface_dev);
}

void NmcliWireless::_clean(const QString &partial)
{
    QProcess cleanProcess;
    // start cleanining process
    cleanProcess.start("bash", QStringList() << "-c" <<
                       QString("nmcli --fields UUID,NAME con list | grep %1").arg(partial));
...
bool NmcliWireless::_errorInResponse(const QByteArray &response)
{
...
bool NmcliWireless::connect(const QString &ssid, const QString &password)
{
...
QString NmcliWireless::current()
{
    QProcess statProcess;
    // start the info command
    statProcess.start("bash", QStringList() << "-c" << QString("nmcli con status | grep %1")
                      .arg(_interface));
    // if process stucked for long time
...
QStringList NmcliWireless::scan()
{
...
QStringList NmcliWireless::interfaces()
{
...
QString NmcliWireless::interface(const QString &interface_dev)
{
...
void NmcliWireless::power(bool turnState)
{
    if(turnState)
        QProcess::execute("nmcli nm wifi on");
    else
        QProcess::execute("nmcli nm wifi off");
}
